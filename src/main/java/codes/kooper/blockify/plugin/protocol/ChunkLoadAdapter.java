        ...
        if (event.getPacketType() == PacketType.Play.Server.CHUNK_DATA) {
            ...
            for (Stage stage : stages) {
                ...
                if (stage.getChunks().contains(new BlockifyChunk(chunkX, chunkZ))) {
                    ...
                    BlockifyLibrary.getInstance().getBlockChangeManager().getExecutorService().submit(() -> BlockifyLibrary.getInstance().getBlockChangeManager().sendChunkPacket(player, blockifyChunk, false));
                }
            }
        }
    }
