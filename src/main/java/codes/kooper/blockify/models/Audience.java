...
public class Audience {
    ...
    public Set<Player> getOnlinePlayers() {
        ...
        for (UUID player : players) {
            Player p = BlockifyLibrary.getInstance().getServer().getPlayer(player);
            if (p != null) {
                onlinePlayers.add(p);
            }
        }
    ...
    public void setMiningSpeed(UUID player, float speed) {
        if (speed < 0 || speed == 1) {
            BlockifyLibrary.getInstance().getLogger().warning("Invalid mining speed for player " + player + ": " + speed);
            return;
        }
