...
public class View {
    ...
    public void removeBlock(BlockifyPosition position) {
        ...
        for (Player viewer : stage.getAudience().getOnlinePlayers()) {
            BlockifyAPI.getInstance().getBlockChangeManager().applyBlockChange(viewer, chunk, position, null, this.name);
        }
    }
    ...
    public void addBlock(BlockifyPosition position) {
        ...
        for (Player viewer : stage.getAudience().getOnlinePlayers()) {
            BlockifyAPI.getInstance().getBlockChangeManager().applyBlockChange(viewer, chunk, position, newData, this.name);
        }
    }
    ...
    public void setBlock(BlockifyPosition position, BlockData blockData) {
        if (hasBlock(position)) {
            ...
            for (Player viewer : stage.getAudience().getOnlinePlayers()) {
                BlockifyAPI.getInstance().getBlockChangeManager().applyBlockChange(viewer, chunk, position, blockData, this.name);
            }
        }
    }
